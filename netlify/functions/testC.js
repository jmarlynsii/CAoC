...
function sendEmail(client, message, senderEmail, senderName, email, attachment, filename) {
    return new Promise((fulfill, reject) => {
        const data = {
            ...
            subject: `New Submission: ${message} Arrival Form`,
            to: email,
...
exports.handler = function(event, context, callback) {
    ...
    const { message, email, attachment, filename } = body;

    // Check if all required fields are provided
    if (!message || !email || !attachment || !filename) {
